...
def simple_rhythm_check():
    ...
    try:
        ...
        if len(shipped_check) > 0 and len(order_check) == 0:
            print("   ❌ ISSUE: Shipped data exists but NO ORDER DATA found!")
        elif len(shipped_check) > 0 and len(order_check) > 0:
            matches = order_check[order_check['has_cla'] == 'MATCH'] if len(order_check) > 0 else pd.DataFrame()
            if len(matches) == 0:
                print("   ❌ ISSUE: Order data exists but NO MATCHING ALIAS/RELATED ITEM values!")
                print(f"   Available ALIAS/RELATED ITEM values: {order_check['ALIAS/RELATED ITEM'].unique()}")
            else:
                print("   ✅ Both shipped and order data exist with matching values")
