...
def fetch_monday_record():
    ...
    board_data = gql(board_query)["boards"][0]    # Fetch specific item
    item_id = 9378053930
    item_query = f'''
    query GetBoardItems {{
      boards(ids: {BOARD_ID}) {{
...
def prepare_record_for_insert(record):
    ...
    print(f"🔄 Found numeric columns: {[col for col in numeric_columns if col in df.columns]}")    # Process date columns - Monday.com format is YYYY-MM-DD
