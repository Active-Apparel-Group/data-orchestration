...
class UniversalMondayUpdater:
    ...
    def load_query_from_config(self, query_config: dict) -> str:
        ...
        if file_path:
            # Support relative to repo root or absolute
    ...
    def load_graphql_template(self, template_name: str) -> str:
        """Load GraphQL template from sql/graphql/mutations/ using global repo_root"""
        template_path = repo_root / "sql" / "graphql" / "mutations" / "monday" / f"{template_name}.graphql"
    ...
    def load_board_metadata(self, board_id: int) -> dict:
        """Load board metadata from configs/boards/ using global repo_root"""
