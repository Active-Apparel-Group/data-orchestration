...
class OrderListTransformer:
    ...
    def run_transform_server_side_optimized(self) -> Dict[str, Any]:
        ...
        print(f"Strategy: Let SQL Server handle data cleaning, avoid 41M cell Python operations")
        print(f"DDL Path: {self.ddl_file_path}")
        ...
        print(f"✅ Created {self.staging_table} with proper schema")
        ...
        for idx, row in raw_tables_df.iterrows():
            ...
            try:
                ...
                print(f"   ✅ Inserted {rows_affected} rows (server-side cleaned)")
        ...
        if successful_customers:
            ...
            try:
                ...
                print(f"✅ Atomic swap completed successfully")
            except Exception as e:
    ...
    def run_transform_single_customer(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if df.empty:
                print(f"WARNING: No valid data found for {customer_name}")
            ...
            print(f"Sample data preview (first 3 rows):")
            ...
            print(f"SUCCESS: {customer_name}: {len(df)} rows processed in {duration:.2f}s")
            return result
            
        except Exception as e:
            self.logger.error(f"Transform failed for {customer_name}: {e}")
            print(f"ERROR: {customer_name}: Transform failed - {e}")
    ...
    def run_transform_all_customers_schema_aware(self) -> Dict[str, Any]:
        ...
        print(f"Tables found: {', '.join(raw_tables_df['TABLE_NAME'].tolist())}")
        print(f"DDL Path: {self.ddl_file_path}")
        ...
        if all_data:
            ...
            print(f"[DEBUG] INSPECTING {len(all_data)} DataFrames before concatenation...")
            for i, df in enumerate(all_data):
                ...
                print(f"[DEBUG] DataFrame {i+1}: {customer_name}")
                ...
                if df.columns.duplicated().any():
                    ...
                    print(f"   ERROR: Individual DataFrame has duplicates: {unique_duplicates}")
                    ...
                    print(f"   ERROR: Column frequencies: {multiple_cols}")
                ...
                print(f"   Columns (first 10): {column_sample}")
            ...
            if len(all_data) > 1:
                ...
                for i, df in enumerate(all_data[1:], 1):
                    ...
                    if current_columns != reference_columns:
                        ...
                        print(f"   ERROR: DataFrame {i+1} column mismatch!")
                        if missing_in_current:
                            print(f"   Missing: {list(missing_in_current)}")
                        if extra_in_current:
                            print(f"   Extra: {list(extra_in_current)}")
                        raise ValueError(f"DataFrame {i+1}: Column structure inconsistent with reference DataFrame")
                
                print(f"   SUCCESS: All {len(all_data)} DataFrames have identical column structure")
                print(f"   Reference columns: {len(reference_columns)} columns")
            ...
            try:
                ...
                print(f"[DEBUG] Attempting pandas concatenation...")
                ...
                print(f"[ERROR] Concatenation failed: {concat_error}")
                print(f"[ERROR] This suggests the issue is in pandas concat(), not our DataFrames")
                print(f"[SUGGESTION] Try run_transform_incremental_customers() to avoid concatenation")
                return {
                    "success": False, 
            ...
            print(f"[INFO] Consolidated DataFrame info:")
            ...
            print(f"   Sample types: {type_samples}")
            ...
            if pipeline_result['success']:
                print(f"[SUCCESS] SCHEMA-AWARE PIPELINE SUCCESS")
                print(f"   {total_rows:,} rows loaded with proper data types")
                print(f"   INT columns preserved as INT (not NVARCHAR)")
                print(f"   DECIMAL columns preserved as DECIMAL (not NVARCHAR)")
                print(f"   Atomic swap completed with correct schema")
            else:
                print(f"[ERROR] SCHEMA-AWARE PIPELINE FAILED: {pipeline_result.get('error')}")
    ...
    def run_transform_incremental_customers(self) -> Dict[str, Any]:
        ...
        print(f"Strategy: Process each customer directly to staging, avoid massive DataFrame consolidation")
        ...
        for idx, row in raw_tables_df.iterrows():
            ...
            try:
                ...
                if result["success"] and result.get("rows_processed", 0) > 0:
                    ...
                    successful_customers.append(result)
                    print(f"   SUCCESS: {len(customer_df)} rows loaded to staging")
        ...
        if successful_customers:
            ...
            try:
                ...
                if swap_result['success']:
                    print(f"[SUCCESS] Atomic swap completed successfully")
                else:
                    print(f"[ERROR] Atomic swap failed: {swap_result.get('error')}")
    ...
    def run(self) -> Dict[str, Any]:
        ...
        print("=" * 80)
        print("[PROCESS] ORDER_LIST TRANSFORM - PRODUCTION DDL-BASED PIPELINE")
        print("=" * 80)
        print("Key Innovation:")
        print("- Creates swp_ORDER_LIST from DDL schema (not broken production table)")
        print("- Enforces proper data types: INT stays INT, DECIMAL stays DECIMAL")
        print("- Eliminates NVARCHAR pollution")
        print("- Maintains atomic swap capabilities")
        print("- ONE-TO-ONE canonical mapping: no duplicate columns possible")
        
        # TRY SERVER-SIDE OPTIMIZED APPROACH FIRST!
        print("\n[STRATEGY] Using SERVER-SIDE OPTIMIZED processing")
        print("- All data cleaning happens in SQL Server")
        print("- No 41M cell Python operations")
        print("- Direct INSERT INTO ... SELECT with transforms")
        
        result = self.run_transform_server_side_optimized()
        
        print(f"\n{'='*80}")
        print("[INFO] FINAL SUMMARY - PRODUCTION RESULTS")
        print(f"{'='*80}")
        
        if result["success"]:
            print(f"[SUCCESS] Production transform completed successfully!")
            print(f"[PROCESS] Server-side optimized: {result.get('server_side_optimized', False)}")
            print(f"[INFO] Customers processed: {result['total_customers_processed']}")
            print(f"[INFO] Total rows: {result['total_rows']:,}")
            print(f"[INFO] Duration: {result['duration']:.2f}s")
            print(f"[INFO] Performance: {result['total_rows'] / result['duration']:.0f} rows/sec")
            print(f"[INFO] Schema preservation: Proper data types enforced")
            
            if result.get('successful_customers'):
                print(f"[SUCCESS] Successful: {', '.join(result['successful_customers'])}")
            
            if result.get('failed_customers'):
                print(f"[ERROR] Failed: {', '.join(result['failed_customers'])}")
                
        else:
            print(f"[ERROR] Production transform failed: {result.get('error', 'Unknown error')}")
